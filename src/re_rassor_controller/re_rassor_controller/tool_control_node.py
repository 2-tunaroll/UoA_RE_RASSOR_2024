...
class ToolControlNode(Node):
    def __init__(self):
        ...
        self.subscription_2 = self.create_subscription(BucketDrum, 'bucket_drum_cmd',
                                                       self.bucket_drum_callback, 10)
        self.subscription_3 = self.create_subscription(Int16, 'vibrating_motor_cmd',
                                                       self.vibrating_roller_callback, 10)
    ...
    def initialise_board(self, board_id):

        board = self.board

        while board.begin() != board.STA_OK:    # Board begin and check board status
            self.print_board_status(board)
            print(f'{board_id} board begin failed')
            time.sleep(2)
